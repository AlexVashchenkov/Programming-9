...
let o = open_out "out.txt";;

let rec print_tree tree depth = 
	match tree with
 Leaf x -> print_string ("Leaf " ^ x)
|Node (t1,t2) -> print_string ("Node (\n" ^ (String.make (depth + 6) ' '));(print_tree t1 (depth + 6)) ;print_string ("\n"  ^ (String.make (depth+6) ' '));(print_tree t2 (depth + 6));print_string ("\n" ^ (String.make (depth+6) ' ') ^ ")");;
                                
print_tree (hafmann (rev lst)) 0;;
